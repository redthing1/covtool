...
def _generate_coverage_data(coverage: CoverageSet, filename: str, module_filter: str = None) -> Dict[str, Any]:
    ...
    if coverage.blocks:
        ...
        block_sizes = [block.size for block in coverage.blocks]
        total_size = sum(block_sizes)
        avg_size = total_size / len(coverage.blocks)
    ...
    if by_module:
        ...
        for module_name, blocks in sorted_modules:
            ...
            if len(data["sample_blocks"]) < 5:
                sample_blocks = sorted(blocks, key=lambda b: b.offset)[:3]
                data["sample_blocks"][module_name] = []
                for block in sample_blocks:
                    abs_addr = module_obj.base + block.offset if module_obj else None
                    block_data = {
...
def print_detailed_info_rich(coverage: CoverageSet, filename: str, module_filter: str = None):
    ...
    # summary statistics
    summary_table = Table(title="[bold]Summary Statistics[/bold]", show_header=False, box=None)
    ...
    if "total_coverage_size" in summary:
        summary_table.add_row("Total Coverage Size", f"{summary['total_coverage_size']:,} bytes")
        summary_table.add_row("Average Block Size", f"{summary['average_block_size']} bytes")
        summary_table.add_row("Block Size Range", f"{summary['min_block_size']} - {summary['max_block_size']} bytes")
    ...
    if data["address_space"]:
        ...
        addr_table = Table(title="[bold]Address Space[/bold]", show_header=False, box=None)
        addr_table.add_column("Property", style="bold")
        addr_table.add_column("Value", style="green")
        
        addr_table.add_row("Address Range", f"{addr_space['min_address']} - {addr_space['max_address']}")
        addr_table.add_row("Span", f"{addr_space['range_bytes']:,} bytes ({addr_space['range_mb']} MB)")
    ...
    if data["sample_blocks"]:
        ...
        for module_name, blocks in list(data["sample_blocks"].items())[:3]:
            ...
            for block in blocks:
                addr_info = f" → {block['absolute_address']}" if "absolute_address" in block else ""
                tree.add(f"{block['offset']} ({block['size']}b){addr_info}")
            console.print(tree)
        console.print()


def print_detailed_info_json(coverage: CoverageSet, filename: str, module_filter: str = None):
