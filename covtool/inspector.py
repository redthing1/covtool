...
class CoverageInspector:
    ...
    def _create_modules_view(self):
        ...
        if len(items) > self.HEADER_ROWS:
            self.module_listbox.set_focus(self.HEADER_ROWS)
    ...
    def _format_module_line(self, mod_info):
        ...
        if self.filtered_coverage.data.has_hit_counts():
            ...
            return f"{mod_name:<{self.MODULE_WIDTH_WITH_HITS}} {mod_info['count']:<8,} {size_str:<10} {percentage_str:<8} {hits_str:<10}"
        else:
            percentage_str = f"{mod_info['percentage']:.1f}%"
            return f"{mod_name:<{self.MODULE_WIDTH_NO_HITS}} {mod_info['count']:<10,} {size_str:<12} {percentage_str:<8}"
    ...
    def _create_blocks_view(self):
        ...
        hit_col_header = (
            f"Hits{sort_indicator}" if self.block_sort_mode == "hits" else "Hits"
        )
        addr_col_header = (
            f"Offset{sort_indicator}" if self.block_sort_mode == "address" else "Offset"
        )

        header_text = f"{'Module':<{self.BLOCK_MODULE_WIDTH}} {addr_col_header:<{self.OFFSET_FIELD_WIDTH}} {'Address':<{self.ADDRESS_FIELD_WIDTH}} {'Size':<{self.SIZE_FIELD_WIDTH}} {hit_col_header:<{self.HITS_FIELD_WIDTH}}"
        header = urwid.AttrMap(urwid.Text(header_text), "header")
        items.append(header)
        ...
        for block_info in self.block_list[:max_blocks]:
            ...
            mod_name = self._truncate_module_name(
                mod_name, has_hits=False
            )  # blocks view doesn't depend on hit count presence

            line_text = f"{mod_name:<{self.BLOCK_MODULE_WIDTH}} 0x{block.start:08x} {abs_addr_str:<{self.ADDRESS_FIELD_WIDTH}} {block.size:<{self.SIZE_FIELD_WIDTH}} {hits_str:<{self.HITS_FIELD_WIDTH}}"
    ...
    def _handle_input(self, key):
        ...
        if key in ("q", "Q"):
            ...
            self._show_hitcount_range_filter_dialog()
        elif key == "z" and self.current_view == "blocks":
            self._show_size_filter_dialog()
        elif key == "/":
    ...
    def _show_hitcount_filter_dialog(self):
        ...
        class HitCountDialog(BaseDialog):
            ...
            def show(self):
                ...
                instructions = [
                    urwid.Text(
                        "Filter blocks by exact hit count (ESC=Cancel, Enter=Apply):"
                    ),
                    urwid.Text(
                        "Enter a number to show only blocks with that hit count"
                    ),
                ]
        ...
        dialog = HitCountDialog(self)
        dialog.show()

    def _show_hitcount_range_filter_dialog(self):
        """Show dialog to filter blocks by hit count range"""
        ...
        def on_ok(_button):
            ...
            if filter_text == "":
                ...
                parsed_filter = self._parse_range_filter(filter_text)
                if parsed_filter:
                    self.hitcount_range_filter = parsed_filter
                # If invalid, keep current filter
            self._refresh_block_list()
            self._update_view()
            self._update_header_footer()
            self.main_loop.widget = self.main_widget
        ...
        edit = urwid.Edit("Hit count range: ", current_filter)
        ok_button = urwid.Button("Apply", on_ok)
        clear_button = urwid.Button("Clear", on_clear)
        cancel_button = urwid.Button("Cancel", on_cancel)

        instructions = [
            urwid.Text("Filter blocks by hit count range:"),
        ...
        dialog = urwid.Filler(
            urwid.LineBox(pile, title="Filter by Hit Count Range"), valign="middle"
        )
        overlay = urwid.Overlay(
            dialog,
            self.main_widget,
    ...
    def _show_size_filter_dialog(self):
        """Show dialog to filter blocks by size range"""
        ...
        def on_ok(_button):
            ...
            if filter_text == "":
                ...
                parsed_filter = self._parse_range_filter(filter_text)
                if parsed_filter:
                    self.size_filter = parsed_filter
                # If invalid, keep current filter
            self._refresh_block_list()
            self._update_view()
            self._update_header_footer()
        ...
        edit = urwid.Edit("Block size range: ", current_filter)
        ok_button = urwid.Button("Apply", on_ok)
        clear_button = urwid.Button("Clear", on_clear)
        cancel_button = urwid.Button("Cancel", on_cancel)

        instructions = [
            urwid.Text("Filter blocks by size range (in bytes):"),
        ...
        dialog = urwid.Filler(
            urwid.LineBox(pile, title="Filter by Block Size Range"), valign="middle"
        )
        overlay = urwid.Overlay(
            dialog,
            self.main_widget,
    ...
    def _show_search_dialog(self):
        """Show search dialog for filtering modules and blocks"""
